...
//            std::cout << "-------------------------\n";
            ...
            for (const auto& cmd : shell_commands) {
                //std::cout << count << "\n";
                count++;
                //std::cout << "Next mode is " << cmd.next_mode << "\n";
                int temp = pipe(prevpipe);
                if (temp < 0) {
...
//                    std::cout << "In child\n";
                    if (cmd.cin_mode == istream_mode::pipe && prev != 0) {
//                        std::cout << "istream mode in pipe\n";
//                        close(prevpipe[1]);
                        dup2(prev, 0);
//                        close(prevpipe[0]);
                    }
                    
                    if (cmd.cout_mode == ostream_mode::pipe && prevpipe[1] != 1) {
//                        std::cout << "ostream mode pipe\n";
//                        close(prevpipe[0]);
                        dup2(prevpipe[1], 1);
//                        close(prevpipe[1]);
                    }
...
//                        std::cout << "appending\n";
                        int output_desc = open(cmd.cout_file.c_str(), O_WRONLY | O_APPEND | O_CREAT, 0666);
                        dup2(output_desc, 1);
                        close(output_desc);
                    }
...
//                        int output_desc = fileno(output_file);
                    ...
                    if (succ == 0 && cmd.next_mode == next_command_mode::on_fail) {
                        exit(0);
                    }
...
//                    close(prevpipe[1]);
//                    if (cmd.cin_mode == istream_mode::pipe) {
//                        close(prevpipe[0]);
//                    }
